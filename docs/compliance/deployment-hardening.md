...
**On-behalf-of delegation for a governed UI (`delegatingSubjects`).** A browser chat UI (e.g. SecAssist/LibreChat) logs each person in with their own OIDC identity, but authenticates to SecRouter with a single service token — it cannot forward each user's own bearer. To keep governance **per end-user** rather than collapsing all traffic onto one service principal, add the UI's service `sub` to `security.oidc.delegatingSubjects` (normally also a `serviceSubject`, so its non-interactive token clears `requireMfa`). When that service calls, SecRouter reads the signed-in user from `X-Sec-Acting-User` (and optional `X-Sec-Acting-Groups`) and **replaces the principal with that end-user** — policy (`policy.users[<user>]` / groups), budgets, quotas, and the usage ledger all attribute to the real person, and the `auth.success` audit records `delegatedBy: <service sub>` (RFC 8693 `act`) for a complete actor→subject chain, surfaced in the console's Audit tab as "*user* · via *service*". The header is honored **only** when the authenticated caller is a listed delegator — an ordinary user's token that sets the header is ignored, so there is no impersonation path. The trust anchor is the service credential itself: scope it to the one UI, protect it like any secret, and enforce the end-user's MFA at the UI's own OIDC login (SecSSO). A forwarded identity is shape-checked (control chars / length) before use. Header names are overridable via `actingUserHeader` / `actingGroupsHeader`. Absent/empty ⇒ no delegation.
...
- [ ] `security.oidc.delegatingSubjects` (if used) lists only trusted governed UIs; each forwards the signed-in user via `X-Sec-Acting-User` so policy and audit stay per end-user, and its service credential is protected/rotated like any secret
